...
public final class StringUtil {
	...
	public static String toUTFbaString(String s) {
		...
		for (int i = 0; i < s.length(); i++) {
			...
			if (c >= 0 && c <= 255) {
				...
				/*
				 * byte[] b; try { b = Character.toString(c).getBytes("UTF-8");
				 * } catch (Exception ex) { b = new byte[0]; } for (int j = 0; j
				 * < b.length; j++) { int k = b[j]; if (k < 0) { k += 256; }
	 ...
	 */
	/*
	 * public static String decodeUnicode(String theString) { char aChar; int
	 * len = theString.length(); StringBuffer outBuffer = new StringBuffer(len);
	 * for (int x = 0; x < len;) { aChar = theString.charAt(x++); if (aChar ==
	 * '\\') { aChar = theString.charAt(x++); if (aChar == 'u') { // Read the
	 * xxxx int value = 0; for (int i = 0; i < 4; i++) { aChar =
	 * theString.charAt(x++); switch (aChar) { case '0': case '1': case '2':
	 * case '3': case '4': case '5': case '6': case '7': case '8': case '9':
	 * value = (value << 4) + aChar - '0'; break; case 'a': case 'b': case 'c':
	 * case 'd': case 'e': case 'f': value = (value << 4) + 10 + aChar - 'a';
	 * break; case 'A': case 'B': case 'C': case 'D': case 'E': case 'F': value
	 * = (value << 4) + 10 + aChar - 'A'; break; default: throw new
	 * IllegalArgumentException( "Malformed   \\uxxxx   encoding."); } }
	 * outBuffer.append((char) value); } else { if (aChar == 't') { aChar =
	 * '\t'; } else if (aChar == 'r'){ aChar = '\r'; } else if (aChar == 'n'){
	 * aChar = '\n'; } else if (aChar == 'f'){ aChar = '\f'; }
	 * 
	 * outBuffer.append(aChar); } } else { outBuffer.append(aChar); }
	 * 
	 ...
	 * @throws
	 */
	// public static String getMbType(String mobile) throws
	// FroadBusinessException {
	// if (!StringUtil.isMobileNumber(mobile)) {
	// throw new FroadBusinessException("非手机号码");
	// } else {
	 ...
	 * @throws
	 */
	// public static String hiheAcctNo(String acctNo) throws
	// FroadBusinessException {
	// try {
	// return acctNo.substring(0, 4) + "*********" +
	// acctNo.substring(acctNo.length() - 4);
	// } catch (Exception e) {
	// LogCvt.error(e.getMessage(), e);
	// throw new FroadBusinessException("用户帐号有误！");
	// }
	// }
	...
	public static String fixingSplitStr(String fileSplitStr) {
		return fileSplitStr.replaceAll("\\{", "\\\\{")
	 ...
	 * @param strings
	 * @return
	 * @Description:根据短信模板id来组装要发送的短信内容
	 * @param 2018年4月10日
	 */
	public static String splitSmsTemplate(String contentTemp,SmsNotificationReq req)throws NullPointerException{
		/**
		 * 最多四个参数
		 */
		String[] tempVolititle=new String[4];
		tempVolititle[0]=req.getValue1();
		tempVolititle[1]=req.getValue2();
		tempVolititle[2]=req.getValue3();
		tempVolititle[3]=req.getValue4();
		
		for (int i = 1; i < tempVolititle.length+1; i++) {
			contentTemp=contentTemp.replace("@"+i+"@", tempVolititle[i-1]);
		}
		return contentTemp;
	}
	/**
	 ...
	 * @param 2018年4月11日
	 */
	public static String indexSmsTemp(String indexTemp){
		Map<String,String> map=ContantsApi.mapSms;
		return map.get(indexTemp);
	}
	
	public static void main(String[] args) {
		StringBuffer sb=new StringBuffer();
		String result=null;
		String content="{\"name\":\"@1@\",\"code\":\"@2@\",\"minute\":\"@3@\"}";
		String[] sa=new String[]{"xx","yy","zz","88"};
		//System.out.println(splitSmsTemplate(content,sa));
	}
}
